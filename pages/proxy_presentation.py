...
with st.expander("Как пользоваться?"):
    st.markdown(
        """
1. В поле "Введите пароль" — введите пароль для доступа к счёту.
__Правильный пароль: admin123__

2. Нажмите кнопку "Проверить баланс" — чтобы проверить доступ к информации.

3. Если пароль верный, вы увидите баланс счёта (1000).
Если неверный — появится сообщение "Доступ запрещен".
        """
    )
